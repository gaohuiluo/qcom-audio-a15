...
**关联数据类型**#gpr_packet_t  ****否则返回错误（参见）—— 包的所有权归还给调用者。
...
**详细描述**当数据链路层收到一个包时，它调用此 GPR 函数来处理该包并将其路由到最终目的地。  **依赖**无。
...
**详细描述**当数据链路层完成一个包的发送时，它使用此函数向 GPR 发出信号以释放该包。  **依赖**无。
...
**详细描述**当 GPR 向目的域发送一个包时，它调用对应数据链路层的此函数。  **依赖**无。
...
**详细描述**当 GPR 完成处理从数据链路层接收的包时，它将该包返还给数据链路层，由其释放缓冲区。  **依赖**无。
