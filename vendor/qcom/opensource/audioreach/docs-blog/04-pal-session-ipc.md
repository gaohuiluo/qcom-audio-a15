...
`Session::makeSession`（[Session.cpp](../audioreach-pal/session/src/Session.cpp)）按流类型
造出具体会话：
...
会话的 `open()`（[SessionAlsaPcm.cpp:140](../audioreach-pal/session/src/SessionAlsaPcm.cpp#L140)）
...
`SessionAlsaUtils::open`（[SessionAlsaUtils.cpp:364](../audioreach-pal/session/src/SessionAlsaUtils.cpp#L364)）：
...
（如切换某后端采样率、开关某算法）。见
[SessionAlsaPcm.cpp:349](../audioreach-pal/session/src/SessionAlsaPcm.cpp#L349) 附近：
...
[PayloadBuilder.cpp](../audioreach-pal/session/src/PayloadBuilder.cpp) 的活：
...
接口定义在 [ipc/HwBinders/interfaces/pal/1.0/IPAL.hal](../audioreach-pal/ipc/HwBinders/interfaces/pal/1.0/IPAL.hal)，
一对 client/server 包装：
...
- **client**：[pal_ipc_client/src/pal_client_wrapper.cpp](../audioreach-pal/ipc/HwBinders/pal_ipc_client/src/pal_client_wrapper.cpp)
...
- **callback**：DSP 事件（数据就绪、drain 完成、检测事件）通过 `IPALCallback` 反向回调，
  ...
  （[pal_client_wrapper.cpp:219](../audioreach-pal/ipc/HwBinders/pal_ipc_client/src/pal_client_wrapper.cpp#L219)）。
