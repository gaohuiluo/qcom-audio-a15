...
`gsl_open` → `gsl_graph_open`（`gsl_graph.c:4651`）
→ 对每个 GKV 节点 `gsl_graph_open_single_gkv`（`gsl_graph.c:3290`）。
单个 GKV 的求解过程：
...
- 把外部（客户端）内存映射进来（`gsl_shmem_map_extern_mem`），并做 LRU 缓存避免反复
  映射（`gsl_datapath.c` 的 `ext_mem_cache`）。
