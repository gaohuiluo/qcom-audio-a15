...
`pal_stream_write`（`Pal.cpp:438`）直接转
`s->write(buf)`，`pal_buffer`（`PalDefs.h:854`）
...
`cachedState = STREAM_STARTED` 的处理（`StreamPCM.cpp:481`）
